...
async function safeFetch(url: string, options?: RequestInit): Promise<Response> {
  ...
  try {
    ...
    const message = err instanceof TypeError && err.message === "Failed to fetch"
      ? "Network request failed. The Google Apps Script deployment may need authorization — visit the script URL in your browser first, or check your connection."
      : err instanceof Error ? err.message : "Unknown network error";
    throw new GoogleAppsScriptError(message);
  }
...
export const callSessionAction = async <T>(
  action: "loginUser" | "logoutUser" | "updateLastActive" | "updateUserRole" | "getOnlineUsers" | "createNotification" | "getNotifications" | "markNotificationRead" | "markAllNotificationsRead" | "archiveNotifications" | "cleanupNotifications" | "getCachedMetrics" | "computeAndStoreMetrics" | "getRoles" | "getRoutesForRole",
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  data?: any
