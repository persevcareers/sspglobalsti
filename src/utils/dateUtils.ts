...
// Parses a date string (raw YYYY-MM-DD or formatted) into a Date object at midnight IST (T00:00:00+05:30)
export const parseToISTDate = (dateStr: string): Date | null => {
  if (!dateStr) return null;
  try {
    const cleanDate = dateStr.includes(" - ") ? parseFormattedDate(dateStr) : dateStr;
    if (!cleanDate) return null;
    return new Date(`${cleanDate}T00:00:00+05:30`);
  } catch (e) {
    return null;
  }
};
